# src/rl/train_sac_lagrangian_ims_bearing.py
import os
import json
import time
...
class ReplayBuffer:
    def __init__(self, obs_dim, size=1_000_000):
        ...
        self.act_buf = np.zeros((size,), dtype=np.int64)          # discrete actions
...
class SACConfig:
    ...
    torch_num_threads: int = 4  # Windows speed knob

    # -------- Env (bearing) --------
    runs_pkl: str = "src/data/data/raw/ims_bearing/runs.pkl"
    model_dir: str = "src/data/models/ensemble_rul_bearing"
    rul_min: float = 15.0

    # IMPORTANT: horizon control
    max_steps: int = 300  # set to 300 (truncated) or large like 100000 to allow near full run
    ...
    # every update_every env steps, do updates_per_trigger gradient steps
    ...
    # entropy temperature (fixed for debug)
    ...
    run_dir: str = "runs/ims_bearing/sac_lagrangian_discrete"
...
def main(cfg: SACConfig):
    ...
    env = NASABearingMaintenanceEnv(
        runs_pkl=cfg.runs_pkl,
        model_dir=cfg.model_dir,
        rul_min=cfg.rul_min,
        max_steps=cfg.max_steps,
        c_failure=cfg.c_failure,
        c_operate=cfg.c_operate,
        c_inspect=cfg.c_inspect,
        c_minor=cfg.c_minor,
        c_replace=cfg.c_replace,
        seed=cfg.seed,
    )

    assert isinstance(env.action_space, gym.spaces.Discrete), \
        "Discrete SAC expects env.action_space = gym.spaces.Discrete."
...
if __name__ == "__main__":
    cfg = SACConfig(
        total_steps=300_000,
        run_dir="runs/ims_bearing/sac_lagrangian_discrete",
        # speed knobs
        update_every=200,
        ...
        # IMPORTANT: set this as you like
        max_steps=300,  # or 100000 like you tried for PPO-style long episodes
        rul_min=100.0,  # match what you used in PPO runs if desired
    )
    main(cfg)
