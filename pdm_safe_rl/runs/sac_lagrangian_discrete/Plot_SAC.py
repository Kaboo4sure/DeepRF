...
# ---- NEW: compact y-axis for safety region (and reduce visual staggering) ----
def style_safety_yaxis(ax, limit, y_max=0.06):
    ax.set_ylim(0.0, y_max)
    # emphasize the constraint band
    ax.axhspan(0.0, limit, alpha=0.08, label=None)

# ---- NEW: downsample raw curves to reduce zig-zag clutter ----
RAW_STRIDE = 6  # increase to 10 if still too busy

# ---------- 01 Safety ----------
fig, ax = plt.subplots(figsize=(7.5, 4.5))

ax.plot(iters[::RAW_STRIDE], p_unsafe[::RAW_STRIDE],
        alpha=0.25, linewidth=1.0, label="p_unsafe (raw)")

ax.plot(iters, rolling_mean(p_unsafe, 25),
        linewidth=2.5, label="p_unsafe (roll-25)")

ax.axhline(limit, linestyle="--", linewidth=1.5, label=f"target limit = {limit}")
...
style_xaxis(ax)
style_safety_yaxis(ax, limit, y_max=0.06)

fig.tight_layout()
fig.savefig(os.path.join(out_dir, "01_safety_p_unsafe_sac.png"), dpi=300)
...
ax.plot(iters[::RAW_STRIDE], lambdas[::RAW_STRIDE],
        alpha=0.25, linewidth=1.0, label="lambda (raw)")

ax.plot(iters, rolling_mean(lambdas, 25),
        linewidth=2.5, label="lambda (roll-25)")
...
fig.tight_layout()
fig.savefig(os.path.join(out_dir, "02_lambda_sac.png"), dpi=300)
...
ax.plot(iters[::RAW_STRIDE], returns[::RAW_STRIDE],
        alpha=0.25, linewidth=1.0, label="avg return (raw)")

ax.plot(iters, rolling_mean(returns, 25),
        linewidth=2.5, label="avg return (roll-25)")
...
fig.tight_layout()
fig.savefig(os.path.join(out_dir, "03_return_sac.png"), dpi=300)
...
ax.plot(iters, alpha, linewidth=2.0, label="alpha")
ax.set_xlabel("Environment Steps")
...
fig.tight_layout()
fig.savefig(os.path.join(out_dir, "04_alpha_sac.png"), dpi=300)
...
ax.plot(iters, entropy, linewidth=2.0, label="policy entropy")
ax.set_xlabel("Environment Steps")
...
fig.tight_layout()
fig.savefig(os.path.join(out_dir, "05_entropy_sac.png"), dpi=300)
...
print(f"Raw plotting stride: {RAW_STRIDE} (increase if still busy)")
